...
class RandomPlusBeatSpiedAI(Brain):
    def play_turn(self, player: Player, game: GameStatus, spied_card: Optional[Card]) -> Card:
        ...
        if spied_card and len(player.hand) > 1:
            ...
            best_result = FightResult.blue_wins_game if player.color == Color.red else FightResult.red_wins_game
            best_card = None
            ...
            for card in player.hand:
                ...
                res = QUICK_FIGHT_RESULT[(red_card, blue_card, previous_red_card, previous_blue_card)]
                ...
                is_better = (res >= best_result) if player.color == Color.blue else (res <= best_result)
                if is_better:
                    best_result = res
                    best_card = card
            return best_card
